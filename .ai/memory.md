...
- [事实] `/admin` 登录态由 `lib/services/admin_session_store.dart` 通过 `shared_preferences` 持久化；刷新页面保持已登录，点击 Logout 会清除本地登录态。
...
- [决策] BT 手动配置的 RX/TX 最小时长拆为 `RX_min_us` / `TX_min_us`；`Rmin_us` 已从模型、计算逻辑和 BT seed JSON 移除。
